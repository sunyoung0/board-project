...
export default function UserPage() {
			...
			<div className='my-page-top-container'>
				...
				<div className='my-page-top-info-box'>
					<div className='my-page-info-nickname-container'>
						{ nicknameChange ? (
							<input  className='my-page-info-nickname-input' type='text' value={nickname} onChange={(event) => onNicknameChangeHandler(event.target.value)} size={nickname.length}/>
						) : (
	...
	const MyPageBottom = () => {
		...
		const getPageBoardList = (boardCount:number) => {
			...
			const lastIndex = boardCount > COUNT_BY_PAGE * currentPage ? COUNT_BY_PAGE * currentPage : boardCount;
		...
		//					 effect					 //
		// description : 화면 첫 로드 시 게시물 리스트 불러오기 //
		useEffect(() => {
			...
			<div className='my-page-bottom'>
				...
				<div className='my-page-bottom-container'>
					{ boardCount ? (
						...
						<div className='my-page-bottom-list-nothing'>	게시물이 없습니다. </div>
					)}
				...
				{boardCount !== 0 && <Pagination totalPage={totalPage} currentPage={currentPage} onPageClickHandler={onPageClickHandler} onPreviousClickHandler={onPreviousClickHandler} onNextClickHandler={onNextClickHandler} />}
			</div>
		);
	}
